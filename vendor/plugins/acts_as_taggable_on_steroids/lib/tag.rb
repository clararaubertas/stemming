class Tag < ActiveRecord::Base
  ...
  validates_uniqueness_of :name, :scope => [:object_type]
  ...
  def self.find_or_create_with_like_by_name(name, object_type)
    find(:first, :conditions => ["name ILIKE ? and object_type = ?", name, object_type]) || create(:name => name.downcase, :object_type => object_type)
  end
  ...
  class << self
    ...
    def options_for_counts(options = {})
      ...
      group_by  = "#{Tag.table_name}.id, #{Tag.table_name}.name, #{Tag.table_name}.object_type HAVING COUNT(*) > 0"
      group_by << " AND #{having}" unless having.blank?
      
      { :select     => "#{Tag.table_name}.id, #{Tag.table_name}.name, #{Tag.table_name}.object_type, COUNT(*) AS count", 
        :joins      => joins.join(" "),
